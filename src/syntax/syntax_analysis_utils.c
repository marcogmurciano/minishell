 ...
 *
 * This function scans the provided segment for tokens of type TOKEN_CMD or TOKEN_ARG,
 * allocates a new null-terminated array of strings, and copies the corresponding token
 * values into the array. Handles allocation errors gracefully.
 *
 ...
 * @return A newly allocated, null-terminated array of strings representing the command and its arguments,
 ...
 *
 * This function traverses the given segment of tokens, searching for an output redirection
 * token (TOKEN_REDIR_OUT_FILE or TOKEN_APPEND_FILE) at the end of the segment. If found,
 * it duplicates and returns the output file name. If memory allocation fails, it frees
 * the token segment and calls the minishell's error handler.
 *
 * @param minishell Pointer to the minishell structure for error handling.
 * @param segment Pointer to the head of the token segment.
 * @return A duplicated string of the output file name, or NULL if not found or on error.
 */
 ...
 * (typically representing ">>" in shell syntax) just before the end-of-file token.
 *
 * @param segment Pointer to the head of the token segment.
...
int get_append_status(t_token *segment)
{
	...
	current = segment;
	while (current && current->next != NULL && current->next->token_type != TOKEN_EOF)
		current = current->next;
 ...
 * duplicated string of the next token's value, which is used as the heredoc delimiter.
